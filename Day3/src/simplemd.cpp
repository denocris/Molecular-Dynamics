...
// all of them have a reasonable default value, set in read_input()
  ...
  MPI_Status Status;
  int rank, size;
  int ReplicaRank, ReplicaSize;
  int world_id;

  MPI_Comm_rank(this -> MyComm, &rank);
  MPI_Comm_size(this -> MyComm, &size);
  MPI_Comm_rank(this -> ReplicaComm, &ReplicaRank);
...
// kinetic energy is calculated
  ...
  // exchange section
    ...
    MPI_Sendrecv_replace(tmp_buf, 3, MPI_DOUBLE, partner, rank, partner,
    			     rank, this->ReplicaComm, &Status);

    	if(ReplicaRank > partner)
    	  CheckRand = tmp_buf[2];
    	...
    	if(delta >= 1.){
    	  ...
    	  MPI_Sendrecv_replace(&(positions[0][0]), 3*natoms, MPI_DOUBLE, partner, rank, partner,
    			       rank, this->ReplicaComm, &Status);

    	  MPI_Sendrecv_replace(&(velocities[0][0]), 3*natoms, MPI_DOUBLE, partner, rank, partner,
    			       rank, this->ReplicaComm, &Status);
...
int main(int argc,char*argv[]){
  ...
  int color, NewRank, NewSize, n_comm, rep_color, ReplicaSize, ReplicaRank;
  ...
  if(n_files > 1){
    ...
    if(size % n_files != 0 or size % n_comm != 0){
      if(rank == 0)
	cerr << "\n\tSIZE % n_files != 0 or  SIZE % n_comm != 0\n\texit\n\n" << endl;
    ...
    color = rank / n_comm;
    rep_color = rank % n_comm;

    MPI_Comm_split(MPI_COMM_WORLD, color, rank, &RowComm);
    MPI_Comm_split(MPI_COMM_WORLD, rep_color, rank, &ReplicaComm);

    MPI_Comm_rank(RowComm, &NewRank);
    MPI_Comm_size(RowComm, &NewSize);

    MPI_Comm_rank(ReplicaComm, &ReplicaRank);
    MPI_Comm_size(ReplicaComm, &ReplicaSize);

    fprintf(stderr, "\tStart with %d of %d and go to %d of %d; ReplicaID = %d; ReplicaSize = %d\n",
	    rank, size, NewRank, NewSize, ReplicaRank, ReplicaSize);
  }
  else{
